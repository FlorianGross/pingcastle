...
namespace PingCastle.Healthcheck
{
    public class HealthcheckAnalyzer : IPingCastleAnalyzer<HealthcheckData>
    {
        ...
        private readonly IAccountDataProcessor _accountProcessor = new AccountDataProcessor();

        // TODO : Currently Hard-coded until we break apart the Windows-specific parts.
        ...
        public HealthcheckAnalyzer(IWindowsNativeMethods nativeMethods, IIdentityProvider identityProvider, Smb2ProtocolTest smb2Test)
        {
        ...
        public HealthcheckData GenerateCartoReport(string server, int port, NetworkCredential credential, bool AnalyzeReachableDomains)
        {
            ...
            using (ADWebService adws = new ADWebService(server, port, credential, _identityProvider, _nativeMethods))
            {
        ...
        public HealthcheckData PerformAnalyze(PingCastleAnalyzerParameters parameters)
        {
            ...
            using (ADWebService adws = new ADWebService(parameters.Server, parameters.Port, parameters.Credential, _identityProvider, _nativeMethods))
            {
                domainInfo = adws.DomainInfo;
                if (adws.useLdap)
                {
                    _ui.DisplayWarning("Performance warning: using LDAP instead of ADWS");
                }
            ...
            foreach (var rule in rules.ComputeRiskRules(healthcheckData))
            {
                ...
                risk.Notice = rule.Notice;
                risk.NoticeTooltip = rule.NoticeTooltip;
                                                            ...
                                                            List<ReachableDomainInfo> domainlist)
        {
            ...
            try
            {
                ...
                adws = new ADWebService(domainToExplore, port, credential, _identityProvider, _nativeMethods);
            ...
            {
                _ui.DisplayError("Unable to explore " + domainToExplore + " (" + ex.Message + ")");
        ...
        private void GenerateGeneralData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            healthcheckData.WinTrustLevel = WindowsTrustChecker.CheckWinTrustFlags(Assembly.GetEntryAssembly());
            healthcheckData.Level = PingCastleReportDataExportLevel.Full;

            // Get KDS Root Keys information
            adws.Enumerate(
                domainInfo.ConfigurationNamingContext,
                "(objectClass=msKds-ProvRootKey)",
                new string[] { "distinguishedname" },
                (ADItem aditem) =>
                {
                    healthcheckData.HasKdsRootKey = true;
                });
        }
        ...
        private void GetAzureInfo(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            if (!string.IsNullOrEmpty(healthcheckData.AzureADId))
            {
                adws.Enumerate(domainInfo.DefaultNamingContext,
                                        ...
                                        {
                                            Trace.WriteLine("Microsoft Entra Device Registration enabled");
                                        });
            }
        }
        ...
        private void GenerateUserData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            int approximateUserCount = 0;

            // Extracted from loop for optimization
            //   - the timescale difference is large enough that this won't ultimately matter.
                    ...
                    try
                    {
                        if (x.ObjectSid != null)
                        {
                            ...
                            if (x.ObjectSid.IsWellKnown(System.Security.Principal.WellKnownSidType.AccountGuestSid))
                            {
                                ...
                                if (x.UserAccountControl != 0 && x.IsAccountEnabled())
                                {
                        ...
                        if (!_accountProcessor.ProcessAccount(healthcheckData.UserAccountData, x, false, healthcheckData.DCWin2008Install, healthcheckData.ListHoneyPot))
                        {
                            return;
                        }

                        // Checks deliberately sequenced to process fastest first as an optimization.
                        // Sequence influenced by both operator performance and likelihood of data in AD users
                        // Not using IsNullOrEmpty to avoid function call overhead.
                        if (!x.IsAccountDisabled() &&
                            (x.Mail != null && x.Mail.Length > 0) &&
                            (x.LastLogonTimestamp != DateTime.MinValue && x.LastLogonTimestamp >= sixMonthsAgo) &&
                            (x.ServicePrincipalName == null || x.ServicePrincipalName.Length == 0))
                        {
                            approximateUserCount++;
                        }

                        // only enabled accounts and no guest account
                        if (x.IsAccountEnabled())
                        {
                            // login script
            ...
            healthcheckData.ApproximateUserCount = approximateUserCount;
        }
        ...
        List<HealthcheckScriptDelegationData> CheckScriptPermission(ADWebService adws, ADDomainInfo domainInfo, string file)
        {
            ...
            try
            {
                ...
                foreach (FileSystemAccessRule rule in accessRules)
                {
                    ...
                    if (!TryMatchLowPrivilegeUser(sid, adws, out var userName))
                        continue;
        ...
        {
            // Delegate to AccountDataProcessor (moved for platform-agnostic access)
            var processor = new AccountDataProcessor();
            ...
            WorkOnReturnedObjectByADWS callback =
                (ADItem x) =>
                {
                    try
                    {
                            ...
                            if (x.IsAccountEnabled())
                            {
                                ...
                                if (x.WhenCreated.AddYears(3) <= DateTime.Now && x.LastLogonTimestamp.AddDays(45) > DateTime.Now)
                                {
                                    ...
                                    if (x.PwdLastSet.AddYears(3) < x.LastLogonTimestamp)
                                    {
                                        ...
                                        healthcheckData.ListClusterPwdNotChanged.Add(_accountProcessor.GetAccountDetail(x));
                                    }
                                }
                            }
                        ...
                        string os = OperatingSystemHelper.GetOperatingSystem(x.OperatingSystem);
                        ...
                        if (!_accountProcessor.ProcessAccount(proxy, x, true, healthcheckData.DCWin2008Install, healthcheckData.ListHoneyPot))
                            return;
                        ...
                        if (x.IsAccountEnabled())
                        {
                            ...
                            if (x.DistinguishedName.Contains("OU=Domain Controllers,DC=") || x.PrimaryGroupID == 516 || x.PrimaryGroupID == 521)
                            {
                                ...
                                dc.IsGlobalCatalog = x.msDSIsGC;
                                dc.IsReadOnly = x.msDSisRodc;
                                ...
                                if (!string.IsNullOrEmpty(x.OperatingSystem) && x.OperatingSystem.Contains("Server"))
                                {
                                    ...
                                    if (x.WhenCreated.AddDays(45) <= DateTime.Now && x.LastLogonTimestamp.AddDays(45) > DateTime.Now)
                                    {
                                        ...
                                        if (x.PwdLastSet.AddDays(45) < x.LastLogonTimestamp)
                                        {
                                            ...
                                            healthcheckData.ListComputerPwdNotChanged.Add(_accountProcessor.GetAccountDetail(x));
                                        }
                                    }
                                }
                            ...
                            {
                                bool isActive = _accountProcessor.IsComputerActive(x);
                                bool legacyLapsFound = false;
                                bool newLAPSFound = false;
                                if (x.ReplPropertyMetaData.IsLapsFound(LAPSAnalyzer.LegacyLAPSIntId, out DateTime lastOriginatingChange))
                                {
                                    legacyLapsFound = true;
                                    proxy.AddWithoutDetail("LAPS");
                                    if (isActive)
                                        proxy.AddWithoutDetail("LAPSActive");
                                    if (lastOriginatingChange != DateTime.MinValue)
                                    {
                                        var i = HealthcheckHelper.ConvertDateToKey(lastOriginatingChange);
                                ...
                                if (x.ReplPropertyMetaData.IsLapsFound(LAPSAnalyzer.MsLAPSIntId, out DateTime lastOriginatingChangeUnencrypted)
                                    | x.ReplPropertyMetaData.IsLapsFound(LAPSAnalyzer.MsLAPSEncryptedIntId, out DateTime lastOriginatingChangeEncrypted))
                                {
                                    newLAPSFound = true;
                                    proxy.AddWithoutDetail("LAPSNew");
                                    if (isActive)
                                        proxy.AddWithoutDetail("LAPSNewActive");

                                    var newLAPSDate = lastOriginatingChangeUnencrypted > lastOriginatingChangeEncrypted
                                        ? lastOriginatingChangeUnencrypted
                                        : lastOriginatingChangeEncrypted;

                                    if (newLAPSDate != DateTime.MinValue)
                                    {
                                        var i = HealthcheckHelper.ConvertDateToKey(newLAPSDate);
                                ...
                                if (legacyLapsFound && newLAPSFound)
                                {
                                    proxy.AddWithoutDetail("LAPSBoth");
                                    if (isActive)
                                        proxy.AddWithoutDetail("LAPSBothActive");
                                }
                            }
                        }
                    }
        ...
        private void GenerateGroupSidHistoryData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            WorkOnReturnedObjectByADWS callback =
                (ADItem x) =>
                {
                    _accountProcessor.ProcessSIDHistory(x, healthcheckData.UserAccountData);
                    count++;
                };
        ...
        private void GenerateTrustData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            // Fix for Windows 2000 domains: check CrossRef objects for WITHIN_FOREST trusts
            // where TrustAttributes is zero.
            // https://www.tenable.com/blog/active-directory-trust-misclassification-why-old-trusts-look-like-insecure-external-trusts
            bool hasTrustWithZeroAttributes = healthcheckData.Trusts.Any(t => t.TrustAttributes == 0);
            if (hasTrustWithZeroAttributes)
            {
                var crossRefDnsRoots = new List<string>();
                string[] crossRefProperties = new string[] { "dnsRoot" };
                WorkOnReturnedObjectByADWS callbackCrossRefTrusts =
                    (ADItem x) =>
                    {
                        if (!string.IsNullOrEmpty(x.DnsRoot))
                        {
                            crossRefDnsRoots.Add(x.DnsRoot.ToLowerInvariant());
                        }
                    };
                try
                {
                    adws.Enumerate("CN=Partitions," + domainInfo.ConfigurationNamingContext,
                        ...
                        crossRefProperties, callbackCrossRefTrusts, "OneLevel");
                    Trace.WriteLine("Successfully queried CrossRef objects, found " + crossRefDnsRoots.Count + " domains in forest");
                }
                catch (Exception ex)
                {
                    Trace.WriteLine("Failed to query CrossRef objects for WITHIN_FOREST trust detection: " + ex.Message);
                }

                // Update TrustAttributes for trusts with zero attributes matching CrossRef dnsRoots
                foreach (HealthCheckTrustData trust in healthcheckData.Trusts)
                {
                    if (trust.TrustAttributes == 0 && crossRefDnsRoots.Contains(trust.TrustPartner))
                    {
                        Trace.WriteLine("Updated trust " + trust.TrustPartner + " with WITHIN_FOREST flag (32) based on CrossRef check");
        ...
        private void GeneratePrivilegedGroupAndPermissionsData(ADDomainInfo domainInfo, ADWebService adws, PingCastleAnalyzerParameters parameters)
        {
            ...
            foreach (var user in healthcheckData.AllPrivilegedMembers)
            {
                if (user.IsEnabled)
                {
                    ...
                    {
                        var i = HealthcheckHelper.ConvertDateToKey(user.LastLogonTimestamp);
...
//                    new KeyValuePair<Guid, string>(new Guid("ba33815a-4f93-4c76-87f3-57574bff8109"), "EXT_RIGHT_MIGRATE_SID_HISTORY"),
        ...
        void FilterDelegation(ADItem x, WorkOnDelegation gotDelegation)
        {
            ...
            foreach (ActiveDirectoryAccessRule accessrule in sd.GetAccessRules(true, false, typeof(SecurityIdentifier)))
            {
                ...
                {
                    var message = accessrule.ActiveDirectoryRights switch
                    {
                        _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.GenericAll) => "GenericAll (All Objects)",
                        _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteDacl) => "WriteDacl (All Objects)",
                        _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteOwner) => "WriteOwner (All Objects)",
                        _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.CreateChild) => "CreateChild (All Objects)",
                        _ => null
                    };

                    if (message != null)
                    {
                        gotDelegation((SecurityIdentifier)accessrule.IdentityReference, message);
                    }
                }
                // Object Specific ACEs on msDS-DelegatedManagedServiceAccount class
                Guid DmsaSchemaGuid = new Guid("0feb936f-47b3-49f2-9386-1dedc2c23765");
                if (accessrule.ObjectType == new Guid("00000000-0000-0000-0000-000000000000")
                    && accessrule.InheritedObjectType == DmsaSchemaGuid)
                {
                     var message = accessrule.ActiveDirectoryRights switch
                         {
                             _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.GenericAll) => "GenericAll (Descendant msDS-DelegatedManagedServiceAccount)",
                             _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteDacl) => "WriteDacl (Descendant msDS-DelegatedManagedServiceAccount)",
                             _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteOwner) => "WriteOwner (Descendant msDS-DelegatedManagedServiceAccount)",
                             _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.GenericWrite) => "GenericWrite (Descendant msDS-DelegatedManagedServiceAccount)",
                             _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteProperty) => "WriteProperty (Descendant msDS-DelegatedManagedServiceAccount)",
                             _ => null
                         };
                ...
                if ((accessrule.ObjectFlags & ObjectAceFlags.ObjectAceTypePresent) == ObjectAceFlags.ObjectAceTypePresent)
                {
                    if (accessrule.ObjectType == new Guid("0feb936f-47b3-49f2-9386-1dedc2c23765")) // msDS-DelegatedManagedServiceAccount
                    {
                        if ((accessrule.ActiveDirectoryRights & ActiveDirectoryRights.CreateChild) == ActiveDirectoryRights.CreateChild)
                        {
                            gotDelegation((SecurityIdentifier)accessrule.IdentityReference, "CreateChild msDS-DelegatedManagedServiceAccount Objects");
                        }
                    }
                }

                // Specific handling for BadSuccessor with Descdent Organizational Units
                // Define the GUID for Organizational Unit objects
                Guid ouSchemaGuid = new Guid("bf967aa5-0de6-11d0-a285-00aa003049e2");
                if (accessrule.ObjectType == new Guid("00000000-0000-0000-0000-000000000000")
                    && accessrule.InheritedObjectType == ouSchemaGuid) // Permissions on OU objects
                {
                    // Check for access rule inheritance type - we want descendant objects
                    if (accessrule.InheritanceType == ActiveDirectorySecurityInheritance.Descendents ||
                        accessrule.InheritanceType == ActiveDirectorySecurityInheritance.All)
                    {
                        var message = accessrule.ActiveDirectoryRights switch
                        {
                            _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.GenericAll) => "GenericAll (Descendant OUs)",
                            _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteDacl) => "WriteDacl (Descendant OUs)",
                            _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteOwner) => "WriteOwner (Descendant OUs)",
                            _ when accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.CreateChild) => "CreateChild (Descendant OUs)",
                            _ => null
                        };
        ...
        private void ParseGPOFiles(ADWebService adws, ADDomainInfo domainInfo, Dictionary<string, GPO> GPOList, NetworkCredential credential)
        {
            ...
            catch (UnauthorizedAccessException ex)
            {
                ...
                lock (this)
                {
                    _ui.DisplayError("Exception while generating GPO Data: " + ex.Message);
                }
            }
        ...
        private IReadOnlyDictionary<string, List<string>> GetTemplateList(ADDomainInfo domainInfo, ADWebService adws)
        {
            string[] properties = new string[] {
                        "distinguishedname",
                        "cACertificate",
            ...
            WorkOnReturnedObjectByADWS callback =
                (ADItem x) =>
                {
                    var ca = CreateCertificateAuthority(x, adws);
                    ...
                    if (x.CertificateTemplates != null)
                    {
                        foreach (var ct in x.CertificateTemplates)
                        {
                            if (output.ContainsKey(ct))
                                output[ct].Add(ca.FullName);
                            else
                                output[ct] = new List<string>() { ca.FullName };
                        }
                    }
                };
        ...
        private HealthCheckCertificateAuthorityData CreateCertificateAuthority(ADItem item, ADWebService adws)
        {
            var ca = new HealthCheckCertificateAuthorityData { Name = item.Name, DnsHostName = item.DNSHostName };

            if (item.CACertificate != null)
            {
                ca.CertificatesData = new List<byte[]>();
                foreach (var certificate in item.CACertificate)
                    ca.CertificatesData.Add(certificate.RawData);
            }

            if (!healthcheckData.IsPrivilegedMode)
                return ca;

            var enrollmentRestrictions = "No restrictions";
            if (ca.TryGetEnrollmentRights(out var enrollmentRights))
            {
                enrollmentRestrictions = "Configured (Default)";
                foreach (var entry in enrollmentRights.DiscretionaryAcl.Cast<CommonAce>())
                {
                    if (entry.SecurityIdentifier.Value != "S-1-1-0")
                    {
                        enrollmentRestrictions = "Configured (Custom)";
                        break;
                    }
                }
            }

            ca.EnrollmentRestrictions = enrollmentRestrictions;

            if (!ca.TryGetServerSecurityFromRegistry(out var securityDescriptor))
                return ca;

            ca.IsLowPrivilegedPrincipalOwner = false;
            ...
            if (TryMatchLowPrivilegeUser((SecurityIdentifier)ownerSid, adws, out _))
            {
                ca.IsLowPrivilegedPrincipalOwner = true;
            }

            var enrollPrincipals = new List<string>();
            var managerPrincipals = new List<string>();
            ...
            foreach (ActiveDirectoryAccessRule rule in rules)
            {
                ...
                if (TryMatchLowPrivilegeUser(sid, adws, out var userName))
                {
                    ...
                    {
                        managerPrincipals.Add(userName);
                    }

                    if (rights.HasFlag(CertificationAuthorityRights.Enroll))
                    {
                        enrollPrincipals.Add(userName);
                    }
                }
            }

            if (enrollPrincipals.Any())
                ca.LowPrivelegedEnrollPrincipals = enrollPrincipals;
        ...
        private void GenerateCertificateTemplateData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            WorkOnReturnedObjectByADWS callback =
                (ADItem x) =>
                {
                    ...
                    ct.IsAuthorisedSignaturesRequired = x.AuthorizedSignatures > 0;
                    ...
                    if ((x.msPKICertificateNameFlag & 1) != 0)
                    {
                        ...
                        ct.AllowsToSupplySubjectInRequest = true;
                    }
                    ...
                    if (x.NTSecurityDescriptor != null)
                    {
                        ...
                        if (TryMatchLowPrivilegeUser(owner, adws, out var userName))
                        {
                        ...
                        else
                        {
                            ...
                            // see https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-crtd/211ab1e3-bad6-416d-9d56-8480b42617a4

                            var rights = new Dictionary<string, List<string>>();

                            // look for explicit autorization granted to the user who created the object
                            foreach (ActiveDirectoryAccessRule accessrule in x.NTSecurityDescriptor.GetAccessRules(true, true, typeof(SecurityIdentifier)))
                            {
                                if (accessrule.AccessControlType == AccessControlType.Deny)
                                    continue;
                               
                                if (!TryMatchLowPrivilegeUser((SecurityIdentifier)accessrule.IdentityReference, adws, out var accountName))
                                {
                                    continue;
                                }

                                if(!rights.TryGetValue(accountName, out var accRights))
                                {
                                    accRights = new List<string>();
                                    rights[accountName] = accRights;
                                }

                                if(accessrule.ObjectType == Guid.Empty)
                                {
                                    if (accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.ReadProperty))
                                    {
                                        accRights.Add("READ");
                                    }

                                    if (accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteProperty))
                                    {
                                        accRights.Add("WRITE");
                                    }
                                }

                                if (accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteDacl)
                                    || accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.WriteOwner)
                                    || accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.GenericWrite))
                                {
                                    ct.VulnerableTemplateACL = true;
                                }
                                // all extended rights
                                if (accessrule.ObjectType == EnrollPermission
                                    && (accessrule.ActiveDirectoryRights.HasFlag(ActiveDirectoryRights.ExtendedRight) || ((int)accessrule.ActiveDirectoryRights & 4096) != 0))
                                {
                                    ct.LowPrivCanEnroll = true;
                                    ct.EnrollmentLowPrivilegePrincipals.Add(accountName);

                                    accRights.Add("ENROLL");
                                }
                            }

                            ct.Rights = new List<HealthCheckCertificateTemplateRights>();
                            ct.Rights.AddRange(rights.Select(r => new HealthCheckCertificateTemplateRights
                            {
                                Account = r.Key,
                                Rights = r.Value,
                            }));
                    ...
                    ct.EnrollmentAgentTemplate = ct.EKUs.Contains("1.3.6.1.4.1.311.20.2.1");
                };
        ...
        {
            // see https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-gpsi/92188849-64c8-45dc-8c0c-9cc0ee40e03b
            string[] properties = new string[] {
                        "distinguishedname",
                        "msiFileList",
            };
        ...
        void ThreadGPOAnalysis(int ThreadId, ADWebService adws, string directoryFullName, GPO GPO, ADDomainInfo domainInfo)
        {
            ...
            catch (UnauthorizedAccessException ex)
            {
                lock (this)
                {
                    _ui.DisplayError("Unable to analyze the GPO: " + shortName + "(" + ex.Message + ")");
            ...
            catch (Exception ex)
            {
                lock (this)
                {
                    _ui.DisplayError("Unable to analyze the GPO: " + shortName + "(" + ex.Message + ")");
                    Trace.WriteLine("[" + ThreadId + "] Unable to analyze the GPO: " + shortName + "(" + ex.Message + ")");
                    _ui.DisplayError("More details are available in the trace log (step: " + step + ")");
                    Trace.WriteLine(ex.StackTrace);
                }
            }
        }
        ...
        private void ExtractLogonScripts(ADWebService adws, ADDomainInfo domainInfo, GPO GPO, string gpotarget, RegistryPolReader reader)
        {
            foreach (RegistryPolRecord record in reader.SearchRecord(@"Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Run"))
            {
                ...
                var filePart = _nativeMethods.SplitArguments(filename);
        ...
        {
            int intvalue;
            // https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2003/cc787364%28v%3dws.10%29
            ...
            // https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-2000-server/cc959646(v=technet.10%29
        ...
        {
            string[] asrBasePaths = new[]
            {
                @"Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR",
                @"Software\Policies\Microsoft\Windows Defender Antivirus\Windows Defender Exploit Guard\ASR",
                @"Software\Policies\Microsoft\Microsoft Defender Antivirus\Windows Defender Exploit Guard\ASR"
            };

            foreach (var asrBasePath in asrBasePaths)
            {
                int asr = 0;
                if (reader.IsValueSet(asrBasePath, "ExploitGuard_ASR_Rules", out asr))
                {
                ...
                string rulesPath = asrBasePath + @"\Rules";
                foreach (RegistryPolRecord record in reader.SearchRecord(rulesPath))
                {
                ...
                adws.Enumerate(domainInfo.DefaultNamingContext,
                                        ...
                                        }, (ADItem aditem) =>
                                        {
                                            if (!string.IsNullOrEmpty(aditem.Description))
                                            {
                                                ...
                                                msol.MSOLIsEnabled = aditem.IsAccountEnabled();
            ...
            foreach (var msol in healthcheckData.AzureADConnect)
            {
                adws.Enumerate(domainInfo.DefaultNamingContext,
                                        ...
                                        }, (ADItem aditem) =>
                                        {
                                            ...
                                            msol.ComputerIsEnabled = aditem.IsAccountEnabled();
                                            msol.ComputerPwdLastSet = aditem.PwdLastSet;
                                        });
            }
        }

        private bool TryMatchLowPrivilegeUser(SecurityIdentifier sid, ADWebService adws, out string userName)
        {
            userName = null;

            if (sid.Value == "S-1-1-0")
            {
                userName = GraphObjectReference.Everyone;
                return true;
            }
            else if (sid.Value == "S-1-5-7")
            {
                userName = GraphObjectReference.Anonymous;
                return true;
            }
            else if (sid.Value == "S-1-5-11")
            {
                userName = GraphObjectReference.AuthenticatedUsers;
                return true;
            }
        ...
        private List<KeyValuePair<string, string>> AnalyzeFileSystemSecurity(FileSystemSecurity security, ADWebService adws, bool includeInherited)
        {
            ...
            if (TryMatchLowPrivilegeUser(Owner, adws, out var ownerName))
            {
            ...
            foreach (FileSystemAccessRule accessrule in accessRules)
            {
                ...
                if (!TryMatchLowPrivilegeUser((SecurityIdentifier)accessrule.IdentityReference, adws, out var userName))
                    continue;
                    ...
                    if (pos > 1)
                    {
                        ...
                        foreach (string user in values)
                        {
                            ...
                            if (String.IsNullOrEmpty(user2))
                                user2 = HealthCheckConstants.EmptyUserPrivilege;
                    ...
                    (ADItem x) =>
                    {
                            ...
                            )
                        {
                            if (x.NTSecurityDescriptor != null)
                            {
                                ...
                                if (f)
                                {
                                    healthcheckData.ListLAPSJoinedComputersToReview.Add(_accountProcessor.GetAccountDetail(x));
                                }
                            }
                        }
                    }
            ...
            WorkOnReturnedObjectByADWS callbackAdminSDHolder =
                (ADItem x) =>
                {
                    if (!privilegedUser.Contains(x.DistinguishedName))
                    {
                        ...
                        var w = _accountProcessor.GetAccountDetail(x);
        ...
        private void CheckSmartCard(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            WorkOnReturnedObjectByADWS callbackSmartCard =
                (ADItem x) =>
                {
                    ...
                    {
                        healthcheckData.SmartCardNotOK.Add(_accountProcessor.GetAccountDetail(x));
                    }
                };
        ...
        private void QueryRODCPasswordMetadata(ADWebService adws, ADDomainInfo domainInfo, HealthcheckData healthcheckData)
        {
            try
            {
                string[] properties = new string[] { "replPropertyMetaData" };

                foreach (var dc in healthcheckData.DomainControllers)
                {
                    if (dc.IsReadOnly != true || dc.msDSRevealedUsers == null || dc.msDSRevealedUsers.Count == 0)
                        continue;
                    ...
                    try
                    {
                        ...
                        if (dc.MsDSRevealedUsersMetadata == null)
                            dc.MsDSRevealedUsersMetadata = new Dictionary<string, ADItem.ReplPropertyMetaDataItem>();

                        foreach (var revealedUserDN in dc.msDSRevealedUsers.ToList())
                        {
                            try
                            {
                                var matchingPrivileged = healthcheckData.AllPrivilegedMembers.FirstOrDefault(x => x.DistinguishedName == revealedUserDN);
                                if (matchingPrivileged == null)
                                    continue;

                                rodcConnection.Enumerate(revealedUserDN, "(objectClass=*)", properties,
                                    (ADItem user) =>
                                    {
                                        if (user.ReplPropertyMetaData != null && user.ReplPropertyMetaData.ContainsKey(0x9005A))
                                        {
                                            var currentMetadata = user.ReplPropertyMetaData[0x9005A];
            ...
            {
                Trace.WriteLine($"Error in QueryRODCPasswordMetadata: {ex.Message}");
            }
        }
                        ...
                        if (!SkipNullSession)
                        {
                            ...
                            if (impersonationIdentity != null)
                            {
                                DC.RemoteSpoolerDetected = WindowsIdentity.RunImpersonated(
                                    ...
                                    () => SpoolerScanner.CheckIfTheSpoolerIsActive(dns));
                            }
                            else
                            {
        ...
        private void GenerateRODCData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            adws.Enumerate(domainInfo.DefaultNamingContext, "(primaryGroupID=521)", new string[] { "distinguishedname", "msDS-RevealedUsers", "msDS-RevealOnDemandGroup", "msDS-NeverRevealGroup" },
                (ADItem aditem) =>
                {
                    ...
                    if (aditem.msDSRevealedUsers != null)
                    {
                        ...
                        {
                            var parsed = ADItem.ParseDNWithBinary(u);
                            if (parsed != null)
                            {
                                if (!dc.msDSRevealedUsers.Contains(parsed.DistinguishedName))
                                {
                                    dc.msDSRevealedUsers.Add(parsed.DistinguishedName);
                                    if (parsed.Metadata != null)
                                        dc.MsDSRevealedUsersMetadata[parsed.DistinguishedName] = parsed.Metadata;
                                }
                            }
                        }
                    }
            ...
            Trace.WriteLine("Verifying RODC cached passwords against current metadata");
            QueryRODCPasswordMetadata(adws, domainInfo, healthcheckData);
        }
        ...
        private void GenerateRODCKrbtgtOrphans(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            WorkOnReturnedObjectByADWS callback =
               (ADItem x) =>
               {
                   healthcheckData.RODCKrbtgtOrphans.Add(_accountProcessor.GetAccountDetail(x));
               };

            var filter = "(&(objectclass=user)(!(msDS-KrbTgtLinkBl=*))(sAMAccountName=krbtgt_*)(msDS-SecondaryKrbTgtNumber=*))";
        ...
        private void GenerateDnsData(ADDomainInfo domainInfo, ADWebService adws)
        {
            ...
            GenerateDnsDataForPartition(domainInfo, adws, dnDomainLevel, partitionDN);
            ...
            GenerateDnsDataForPartition(domainInfo, adws, domainInfo.DefaultNamingContext, null);
        ...
        private void GenerateDnsData(ADDomainInfo domainInfo, ADWebService adws, string dn)
        {
            try
            {
                adws.Enumerate(dn, "(objectClass=dnsZone)", new string[] { "distinguishedname", "dnsProperty", "name", "nTSecurityDescriptor" },
                    (ADItem x) =>
                    {
                        // Filter out CNF (conflict) and InProgress replication objects
                        if (x.DistinguishedName.ToLower().Contains("cnf:") || x.DistinguishedName.ToLower().Contains("inprogress"))
                            return;

                        var o = new HealthcheckDnsZones();
                        o.name = x.Name;
                        o.DistinguishedName = x.DistinguishedName;
                        
                        // Determine partition based on DN
                        if (dn.Contains("DC=DomainDnsZones,"))
                        {
                            o.Partition = "DomainDnsZones";
                        }
                        else if (dn.Contains("DC=ForestDnsZones,"))
                        {
                            o.Partition = "ForestDnsZones";
                        }
                        else
                        {
                            o.Partition = "DefaultNamingContext";
                        }
        ...
        private void GenerateDnsDataForPartition(ADDomainInfo domainInfo, ADWebService adws, string dn, string partitionDN)
        {
            // For DNS application partitions (DomainDnsZones, ForestDnsZones), try to use authoritative DC
            if (partitionDN != null && (partitionDN.Contains("DC=DomainDnsZones,") || partitionDN.Contains("DC=ForestDnsZones,")))
            {
                var replicaLocations = GetDnsZoneReplicaLocations(domainInfo, adws, partitionDN);
                if (replicaLocations?.Count > 0)
                {
                    // Check if current DC is authoritative for this partition
                    // The replica locations are machine names, not FQDN, so we grab the short version.
                    var connectedDCShortName = ExtractShortServerName(domainInfo.DnsHostName);
                    if (!string.IsNullOrEmpty(connectedDCShortName) && 
                        replicaLocations.Contains(connectedDCShortName, StringComparer.OrdinalIgnoreCase))
                    {
                        Trace.WriteLine($"Current DC is authoritative for partition {partitionDN}");
                        GenerateDnsData(domainInfo, adws, dn);
                        return;
                    }

                    // Current DC is not authoritative, select a random location.
                    var firstReplica = replicaLocations.TakeRandom();
                    if (!string.IsNullOrEmpty(firstReplica))
                    {
                        try
                        {
                            using (var replicaAdws = new ADWebService(firstReplica, adws.Port, adws.Credential, _identityProvider, _nativeMethods))
                            {
                                Trace.WriteLine($"Using authoritative replica DC {firstReplica} for partition {partitionDN}");
                                GenerateDnsData(domainInfo, replicaAdws, dn);
                                return;
                            }
                        }
                        catch (Exception ex)
                        {
                            Trace.WriteLine($"Failed to connect to replica DC {firstReplica}: {ex.Message}");
                        }
                    }
                }
            }
        ...
        private List<string> GetDnsZoneReplicaLocations(ADDomainInfo domainInfo, ADWebService adws, string partitionDN)
        {
            var replicaLocations = new List<string>();
            try
            {
                var partitionsContainerDN = "CN=Partitions," + domainInfo.ConfigurationNamingContext;
                var filter = $"(&(objectClass=crossRef)(nCName={partitionDN}))";
                var properties = new string[] { "nCName", "msDS-NC-Replica-Locations" };

                adws.Enumerate(partitionsContainerDN, filter, properties, (ADItem partition) =>
                {
                    if (partition.msDSNCReplicaLocations != null)
                    {
                        foreach (string replicaLocationDN in partition.msDSNCReplicaLocations)
                        {
                            // Extract server name from DN like CN=NTDS Settings,CN=SERVER1,CN=Servers,CN=Default-First-Site-Name,CN=Sites,CN=Configuration,DC=domain,DC=com
                            var serverName = ExtractServerNameFromNtdsSettingsDN(replicaLocationDN);
                            if (!string.IsNullOrEmpty(serverName) && !replicaLocations.Contains(serverName, StringComparer.OrdinalIgnoreCase))
                            {
                                replicaLocations.Add(serverName);
                                Trace.WriteLine($"Found DNS zone replica on server: {serverName}");
                            }
                        }
                    }
                });
            ...
            {
                Trace.WriteLine($"Error querying Configuration Partition for DNS zone replicas: {ex.Message}");
            }

            return replicaLocations;
        }

        private string ExtractServerNameFromNtdsSettingsDN(string ntdsSettingsDN)
        {
            const string CN_PREFIX = "CN=";
            const string NTDS_SETTINGS = "NTDS Settings";
            ...
            try
            {
                ...
                // For example:
                // DN format -> CN=NTDS Settings,CN=SERVER1,CN=Servers,CN=Default-First-Site-Name,CN=Sites,CN=Configuration,DC=domain,DC=com
                // We want to extract SERVER1
                var parts = ntdsSettingsDN.Split(',');
                
                // Validate expected DN structure
                if (parts.Length < 2)
                {
                    return null;
                }

                // First part should be "CN=NTDS Settings"
                if (!parts[0].Trim().Equals($"{CN_PREFIX}{NTDS_SETTINGS}", StringComparison.OrdinalIgnoreCase))
                {
                    return null;
                }

                // Second part should be server name
                var serverPart = parts[1].Trim();
                if (!serverPart.StartsWith(CN_PREFIX, StringComparison.OrdinalIgnoreCase))
                {
                    return null;
                }

                var serverName = serverPart.Substring(CN_PREFIX.Length);
                
                return string.IsNullOrWhiteSpace(serverName) ? null : serverName;
            }
            catch (Exception ex)
            {
                Trace.WriteLine($"Error extracting server name from NTDS Settings DN '{ntdsSettingsDN}': {ex.Message}");
                return null;
            }
        }

        private string ExtractShortServerName(string serverName)
        {
            ...
            // If contains dot, take the part before the first dot (short name)
